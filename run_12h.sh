...
trap 'code=$?; echo "[error] run_12h failed at line $LINENO with exit code $code"; exit $code' ERR

# Prevent the 15m and 12h jobs from modifying the repo/db at the same time.
exec 9>"$LOCK_FILE"
flock 9

echo "===== $(date -u '+%Y-%m-%dT%H:%M:%SZ') run_12h start ====="
...
echo "===== $(date -u '+%Y-%m-%dT%H:%M:%SZ') run_12h done ====="
